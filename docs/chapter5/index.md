...
- bash を使用（スクリプトは `$HOME` 配下で作成する想定）
...
定型作業を手作業で繰り返すと、作業時間とヒューマンエラーの増加につながります。
シェルスクリプトにより手順をコード化し、再現性と運用性を高めます。

### シェルスクリプトでできること
- **定期バックアップ**: 毎日自動でファイルを保存
- **ログ監視**: エラーを検知した場合にメール通知
...
シェルスクリプトの習得ポイントは次のとおりです。

1. **基本構文**: 変数、条件分岐、ループを理解する
